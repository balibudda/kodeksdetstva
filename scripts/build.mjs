...
<script type="text/javascript">
  window.kdLoadMetrika = function () {
    ...
    ym(112437788, 'init', {ssr:true, clickmap:true, ecommerce:"dataLayer", referrer: document.referrer, url: location.href, accurateTrackBounce:true, trackLinks:true});
  };
...
<footer class="site-foot">
  ...
  <p class="foot-sibling">Наш сайт-брат — <a href="https://kodeksdeneg.ru" target="_blank" rel="noopener noreferrer">«Кодекс денег»</a>: справочник о долгах, приставах и личных финансах.</p>
  <div class="foot-contacts">
    ...
    <p class="foot-tg"><a href="https://github.com/balibudda/kodeksdetstva-releases/releases/latest/download/kodeksdetstva.apk">📱 Скачать APK для Android</a> — отдельное приложение (не браузер), офлайн-доступ ко всему сайту. Также будет в RuStore.</p>
    <div id="app-version-box" class="app-version-box" hidden>
      <p class="app-version-line">📱 Версия приложения: <b id="app-version-num"></b></p>
      <p class="app-version-update" id="app-update-box" hidden><a id="app-update-link" href="https://github.com/balibudda/kodeksdetstva-releases/releases/latest">🔴 Доступна новая версия — скачать</a></p>
    </div>
  ...
  <p class="foot-mission">Наша миссия — чтобы «красный флаг» замечали раньше беды, а не после неё, и чтобы у разговора о правах ребёнка было место не только тревоге, но и радости.</p>
  ...
  const sectionsHtml = SECTIONS.map(
    (s) => `<li class="sec-card" style="--sec:${s.accent}">
      ...
      <ul class="sec-topics">${topicsOfSection(s.id)
        .slice(0, 6)
        .map((t) => `<li><a href="${topicUrl(t)}"><span class="tc-icon">${topicIconSvg(t.slug, 15)}</span>${esc(t.title)}</a>${t.urgent ? ' <b class="u">срочное</b>' : ''}</li>`)
        .join('')}</ul>
    </li>`,
  ).join('')
...
function renderSection(s) {
  ...
  const list = topics
    .map(
      ...
      <a href="${topicUrl(t)}">
        <span class="tc-body">
          <span class="tc-title"><span class="tc-icon">${topicIconSvg(t.slug, 18)}</span>${esc(t.title)}</span>
...
function renderTebe() {
  ...
  const groupsHtml = groups
    .map(([title, lead, slugs]) => {
      const cards = slugs
        .map((slug) => TOPICS_BY_SLUG[slug])
        .filter(Boolean)
        .map((t) => `<li><a href="${topicUrl(t)}"><span class="tebe-c-t"><span class="tc-icon">${topicIconSvg(t.slug, 15)}</span>${esc(t.title)}</span><span class="tc-go" aria-hidden="true">›</span></a></li>`)
        .join('')
      if (!cards) return ''
  ...
  // Заголовки этих тем на своих страницах написаны с точки зрения родителя
  // («помочь ребёнку», «как поддержать») — здесь, на странице сплошного «ты»,
  // даём свою подпись под тот же голос; ссылка всё равно ведёт на тот же текст.
  const goodSlugs = [
    ['vybor-professii', 'Куда двигаться с профессией — без чужого давления'],
    ['mechty-i-celi-rebenka', 'Твои мечты и цели — как их не потерять'],
    ['muzyka-tvorchestvo-emocii', 'Музыка и творчество — способ прожить эмоции'],
    ['pravo-na-chuvstva', 'Ты имеешь право злиться, бояться и грустить'],
    ['dobrota-i-empatiya', 'Доброта и забота о других — это тоже сила'],
    ['blagodarnost-i-radost-melocham', 'Как замечать хорошее, даже когда вокруг шум'],
  ]
  const goodCards = goodSlugs
    .map(([slug, label]) => [TOPICS_BY_SLUG[slug], label])
    .filter(([t]) => t)
    .map(([t, label]) => `<li><a href="${topicUrl(t)}"><span class="tebe-c-t"><span class="tc-icon">${topicIconSvg(t.slug, 15)}</span>${esc(label)}</span><span class="tc-go" aria-hidden="true">›</span></a></li>`)
    .join('')

  const main = `
${breadcrumbs([{ name: 'Главная', url: '/' }, { name: 'Тебе', url: '/tebe/' }])}
<h1>Тебе — если тяжело, страшно или несправедливо</h1>
<p class="frame">Эта страница для тебя, подросток. Без нотаций и «сам виноват». Здесь коротко: что происходит, какие у тебя права и что можно сделать. Здесь открыто говорят о тяжёлых вещах, но нигде нет описания того, как причинить себе вред, — только как распознать беду и куда обратиться. И это не только про беду — ниже есть и то, что реально может сделать жизнь лучше.</p>
...
<section class="tebe-group">
  <h3>Не только беда: что реально может сделать жизнь лучше</h3>
  <p class="tebe-g-lead">Сюда можно зайти просто так, не потому что случилось что-то плохое.</p>
  <ul class="tebe-cards">${goodCards}</ul>
</section>
...
<p><b>Миссия проекта</b> — чтобы «красный флаг» замечали раньше беды, а не после неё, и чтобы родитель видел в ребёнке
не проект и не собственность, а отдельного человека. И чтобы разговор о правах ребёнка не сводился только к тревоге:
у него есть место и для радости, доверия и обычного счастливого детства — не только для кризисов.</p>
...
Аналитика обезличенная и без записи ваших действий на странице (Вебвизор не используется). Что именно собирается и зачем —
...
<h1>Советник</h1>
<p class="frame">Иногда ситуация с ребёнком не укладывается ни в одну готовую тему на сайте — и хочется просто рассказать всё как есть человеку, а не искать по ключевым словам. Это и есть «Советник»: можно лично написать мне, автору проекта.</p>
<p class="frame"><b>Сразу честно, без обтекаемости:</b> я не психолог, не педагог и не юрист по образованию — просто человек, который вложил много времени в то, чтобы разобраться в правах ребёнка и собрать этот сайт. Если по твоей ситуации я реально что-то знаю или сталкивался с похожим — отвечу и постараюсь помочь. Если нет — так и скажу: «не знаю» или «здесь нужен специалист», и не буду делать вид, что могу больше, чем могу. Это не замена психологу, юристу или горячей линии — в острой ситуации сразу смотри страницу <a href="/pomoshch/">«Помощь сейчас»</a>.</p>
<h2>Как это работает</h2>
<ul>
  <li>Ты пишешь свою ситуацию своими словами;</li>
  <li>Я читаю лично и отвечаю в переписке — без готовых шаблонов;</li>
  <li>Цена не фиксирована и не публикуется здесь — если разговор выходит за рамки простого совета, обсудим это отдельно и лично, а не по умолчанию.</li>
</ul>
...
(function () {
  ...
  form.addEventListener('submit', function (e) {
    ...
    fetch('https://bot.kodeksdetstva.ru/api/messages', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
    }).then(function (r) { return r.ok ? r.json() : Promise.reject(r); })
      .then(function () {
        form.innerHTML = '';
        status.textContent = 'Заявка отправлена. Я отвечу лично на указанный контакт, если смогу помочь — обычно в течение нескольких дней.';
      })
      .catch(function () {
        btn.disabled = false;
  ...
  return layout({
    ...
    description: 'Личное общение с автором проекта «Кодекс детства» по твоей ситуации с ребёнком — честно, без обещаний быть специалистом, которым я не являюсь.',
...
<p class="frame">Действует для сайта <b>kodeksdetstva.ru</b> и всех его версий: Telegram-мини-приложения и бота (<a href="https://t.me/kodeksdetstva_bot/kdbot" target="_blank" rel="noopener noreferrer">t.me/kodeksdetstva_bot/kdbot</a>) и Android-приложения. Последнее обновление: ${esc(BUILD_MONTH)}</p>
...
<p>Бот в Telegram технически — обёртка над тем же Сайтом: принимает команду или текст поиска, отправляет
подходящую страницу в ответ и ничего не сохраняет на своей стороне (нет базы данных). Сообщения передаются через
серверы Telegram — на условиях их собственной политики обработки данных. Android-приложение — тоже обёртка
над Сайтом (то же содержимое в отдельном окне), дополнительных данных сверх описанного в этом разделе не собирает.</p>
...
<p>Сайт использует сервис <b>Яндекс.Метрика</b>, предоставляемый ООО «ЯНДЕКС». Сервис использует cookie для различения визитов; технология записи действий на странице (Вебвизор) на Сайте не используется. Условия обработки данных Яндекс.Метрикой —
...
Отключить сбор можно, запретив cookie в браузере и включив режим «Не отслеживать» в браузере,
...
<p>Обезличенные данные веб-аналитики хранятся в течение срока, установленного сервисом Яндекс.Метрика. Значения в localStorage (отметка о плашке cookie, закладки, история, выбранный регион) хранятся в браузере посетителя до их удаления посетителем.</p>
...
Дата последнего обновления указана в начале страницы.</p>`
  ...
  return layout({
    ...
    description:
      'Как сайт «Кодекс детства» обрабатывает данные: cookie и Яндекс.Метрика для обезличенной статистики (без записи действий), никакой идентификации личности, формы заполняются в браузере.',
...
function renderVseTemy() {
  const rows = TOPICS.map((t, i) => {
    const s = SECTIONS_BY_ID[t.sectionId]
    return `<li style="--sec:${s.accent}"><span class="vt-n">${i + 1}</span><a href="${topicUrl(t)}"><span class="vt-ic">${topicIconSvg(t.slug, 15)}</span>${esc(t.title)}</a>${t.urgent ? ' <b class="u">срочное</b>' : ''}</li>`
  }).join('')
...
async function main() {
  ...
  routes.push(await writePage('/sovetnik/', renderSovetnik()))
